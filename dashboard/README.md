...
- 🩺 **登录体检**:每行按钮,无头恢复已保存会话 → 验证 Rewards 可达 → 读取实时余额 → 回写刷新 cookie,
  结果直接更新账号余额/状态(无需为此跑一次全量任务)
- 🔑 **人工登录**:每行按钮,容器内拉起有头浏览器(Xvfb + noVNC),在内网页面里直接完成微软
  身份验证(如 identity/confirm 卡点),停留在 Rewards 页面 5 秒自动保存会话;noVNC 经看板
  `:8300` 以一次性 token 代理(HTTP + WebSocket),不额外暴露端口,同一时间仅一个会话,20 分钟自动超时
...
- `8300` 看板 Web UI:**无鉴权**,仅限内网访问,不要暴露公网;人工登录的 noVNC 通道由一次性随机
  token 门禁且仅存活于会话期间
