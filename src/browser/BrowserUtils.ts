...
export default class BrowserUtils {
    ...
    async ghostClick(page: Page, selector: string, options?: ClickOptions): Promise<boolean> {
        const isStaleContextError = (e: unknown): boolean => {
        ...
        try {
            ...
            try {
                ...
                // ghost-cursor holds an ElementHandle that can become stale during page navigation.
                // Fall back to native page.click() which re-queries the selector fresh.
                if (isStaleContextError(cursorError)) {
                    this.bot.logger.debug(
                        this.bot.isMobile,
                        'GHOST-CLICK',
                        `ghost-cursor stale context for ${selector}, retrying with native click`
                    )
