...
export class Login {
                ...
                try {
                    ...
                    if (bad && this.bot.compromisedModeActive && this.bot.compromisedReason === 'recovery-mismatch') {
                        ...
                        const incident = {
                            ...
                            docsUrl: this.getDocsUrl('recovery-email-mismatch')
                        }
                ...
                if (/we can't sign you in|we can’t sign you in/i.test(titleText)) {
                    if (!this.bot.compromisedModeActive) {
                        ...
                        const incident = {
                            ...
                            details: ['Microsoft presented a sign-in blocked page during login.'],
                            next: ['Automation paused and global standby engaged.', 'Complete required challenges, then review the docs.'],
                            docsUrl: this.getDocsUrl('we-cant-sign-you-in')
                        }
                        await this.sendIncidentAlert(incident, 'warn')
                        this.startCompromisedInterval()
    ...
    // Unified incident alert sender with clean embed formatting
    private async sendIncidentAlert(
        ...
        // Log full incident block to terminal
        const block = this.buildIncidentBlock(incident)
        this.bot.log(this.bot.isMobile, 'SECURITY', `${block}\nSecurity check by @Light`, 'error', severity === 'critical' ? 'red' : 'yellow')

        try {
            const { ConclusionWebhook } = await import('../util/ConclusionWebhook')
            const color = severity === 'critical' ? 0xFF0000 : 0xFFAA00
            const titlePrefix = severity === 'critical' ? '🚨' : '🔐'
            const fields: Array<{ name: string; value: string; inline?: boolean }> = []
            if (incident.account) {
                fields.push({ name: 'Account', value: `\`${incident.account}\`` })
            }
            if (incident.details && incident.details.length) {
                fields.push({ name: 'Details', value: incident.details.map(d => `• ${d}`).join('\n') })
            }
            if (incident.next && incident.next.length) {
                fields.push({ name: 'Next steps', value: incident.next.map(n => `• ${n}`).join('\n') })
            }
            if (incident.docsUrl) {
                fields.push({ name: 'Docs', value: `[Open troubleshooting](${incident.docsUrl})` })
            }
            await ConclusionWebhook(this.bot.config, '@everyone', {
                embeds: [
                    {
                        title: `${titlePrefix} ${incident.kind}`,
                        description: 'Automation paused for safety. See details below.',
                        color,
                        fields
                    }
                ]
    ...
    // Critical helper removed in favor of sendIncidentAlert
