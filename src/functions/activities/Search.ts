...
export class Search extends Workers {
  ...
  public async doSearch(page: Page, data: DashboardData) {
    ...
    for (let i = 0; i < queries.length; i++) {
      ...
      this.bot.log(
        "SEARCH-BING",
        `${missingPoints} Points Remaining | Query: ${query} | Mobile: ${this.bot.isMobile}`
      )
      ...
      if (maxLoop > 3 && this.bot.isMobile) {
        this.bot.log(
          "SEARCH-BING-MOBILE",
          "Search didn't gain point for 3 iterations, likely bad User-Agent",
          "warn"
        )
        break
      }
      ...
      while (missingPoints > 0) {
        ...
        if (relatedTerms.length > 3) {
          ...
          for (const term of relatedTerms.slice(1, 3)) {
            this.bot.log(
              "SEARCH-BING-EXTRA",
              `${missingPoints} Points Remaining | Query: ${term} | Mobile: ${this.bot.isMobile}`
            )
  ...
  private async bingSearch(searchPage: Page, query: string) {
    for (let i = 0; i < 5; i++) {
      try {
        ...
        if (this.searchCounter >= this.bot.config.searchLimiter) {
          // Message to console indicating the limit has been reached
          this.bot.log(
            "SEARCH-BING",
            `Límite de ${
              this.bot.config.searchLimiter
            } búsquedas alcanzado. Iniciando timeout de ${
              this.bot.config.coldDownTime / 1000
            } segundos.`
          )

          await new Promise((resolve) =>
            setTimeout(resolve, this.bot.config.coldDownTime)
          )

          // End of timeout, message to console indicating the timeout has ended
          this.bot.log(
            "SEARCH-BING",
            "Timeout finalizado. Continuando con las búsquedas."
          )
  ...
  private async getGoogleTrends(
    ...
    while (queryCount > queryTerms.length) {
      ...
      try {
        ...
        for (const topic of data.default.trendingSearchesDays[0]
          ?.trendingSearches ?? []) {
          queryTerms.push({
