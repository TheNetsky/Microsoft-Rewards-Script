            ...
            for (let i = 0; i < articleCount; ++i) {
                ...
                if (gainedPoints <= 0) {
                    this.bot.logger.info(
                        this.bot.isMobile,
                        'READ-TO-EARN',
                        `No points gained, stopping Read to Earn | article=${i + 1}/${articleCount} | status=${response?.status ?? 'unknown'} | oldBalance=${oldBalance} | newBalance=${newBalance}`
                    )
                    break
                }
                ...
                this.bot.logger.info(
                    this.bot.isMobile,
                    'READ-TO-EARN',
                    `Read article ${i + 1}/${articleCount} | status=${response?.status ?? 'unknown'} | gainedPoints=${gainedPoints} | newBalance=${newBalance}`,
                    'green'
                )
